...
public class UserProfileService {
    ...
    public SelfDescriptionDTO getDescription(String email) throws NeighbearsException {
        Optional<Customer> opt= customerRepository.findByEmail(email);
        Customer customer = opt.orElseThrow(() -> new NeighbearsException("User not found"));


        Optional<SelfDescription> optional = profileRepository.findByCustomerId(customer.getId());
        SelfDescription selfDescription = optional.orElseThrow(() -> new NeighbearsException(String.format("Self Description for id %d not  Found", customer.getId())));
    ...
    public SelfDescriptionDTO saveDescription(SelfDescriptionDTO selfDescriptionDTO, Authentication authentication) throws NeighbearsException {
       ...
       // selfDescriptionDTO.setCustomer(customerDTO);
        ...
        Customer customer = optional.orElseThrow(() -> new NeighbearsException(String.format("User not found with email %d", authentication.getName())));
